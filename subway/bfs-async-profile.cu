// Instrumented BFS-async that outputs per-iteration timing CSV:
...
// Mirror of SubgraphGenerator::generate but returns cpu-packing time separately.
// GPU subgraph construction time is lumped into "cpuPacking" since it's part of
// the Subway "subgraph generation" overhead that our Dense Bypass would skip.
template <class E>
...
int main(int argc, char** argv)
{
	...
	// CSV output
	string csvPath = "bfs_async_profile_output.csv";
	...
	while (subgraph.numActiveNodes > 0)
	{
		...
		// --- Phase 1: Partition (trivial cost, included in packing) ---
		...
		for(int i=0; i<partitioner.numPartitions; i++)
		{
			...
			mixLabels<<<partitioner.partitionNodeSize[i]/512 + 1, 512>>>(
			...
			do
			{
				...
				bfs_async<<<partitioner.partitionNodeSize[i]/512 + 1, 512>>>(
			...
			cout << itr << ((itr>1) ? " Inner Iterations" : " Inner Iteration")
			     << " in Global Iteration " << gItr << ", Partition " << i << endl;
		}

		// Compute numActiveEdges for this iteration before generate overwrites
		...
		// --- Phase 1: CPU Packing (subgraph generation for NEXT iteration) ---
		double cpu_packing_ms = generate_and_measure_packing(subgen, graph, subgraph);

		// Write CSV row for this iteration
		csv << gItr << ","
		    << activeNodesBefore << ","
		    << numActiveEdges << ","
		    << cpu_packing_ms << ","
